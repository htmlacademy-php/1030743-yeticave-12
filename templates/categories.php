<main>
    ...
    <div class="container">
        ...
        if ($lots): ?>
            <section class="lots">
                <h2>Все лоты в категории «<span><?= isset($category_name) ? $category_name : ""; ?></span>»</h2>
                <ul class="lots__list">
                    ...
                    <ul class="pagination-list">
                        <?php if (isset($cur_page)) {
                            if ($cur_page > 1): ?>
                                <li class="pagination-item pagination-item-prev"><a
                                            href="category.php?id=<?= htmlspecialchars(strval($_GET['id'])); ?>&page=<?= $cur_page - 1; ?>">Назад</a>
                                </li>
                            <?php else: ?>
                            ...
                            foreach ($pages as $page): ?>
                                ...
                                <?php else: ?>
                                    <li class="pagination-item"><a
                                                href="category.php?id=<?= htmlspecialchars(strval($_GET['id'])); ?>&page=<?= $page; ?>"><?= $page; ?></a>
                                    </li>
                                <?php endif ?>
                            <?php endforeach;
                        } ?>
                        <?php if ($pages_count > $cur_page): ?>
                            <li class="pagination-item pagination-item-next"><a
                                        href="category.php?id=<?= htmlspecialchars(strval($_GET['id'])); ?>&page=<?= $cur_page + 1; ?>">Вперед</a>
                            </li>
                        <?php else: ?>
